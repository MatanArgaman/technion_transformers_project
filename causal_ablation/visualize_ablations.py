...
def validate_total_neurons(experiments):
    ...
    for experiment_name, data in experiments.items():
        ...
        print(f"✓ {experiment_name}: total neurons = {actual_total} (ablated: {len(ablated_neurons)}, cannot_ablate: {len(cannot_ablate_neurons)})")
...
def create_layer_ablation_distribution(experiments, save_path="layer_ablation_distribution.png", show_plot=True):
    ...
    # Stacked bar chart
    ax1.bar(layers, ablated_counts, label='Ablated', alpha=0.7)
    ax1.bar(layers, cannot_ablate_counts, bottom=ablated_counts, label='Cannot Ablate', alpha=0.7)
    ax1.set_xlabel('Layer')
    ax1.set_ylabel('Neuron Count')
    ax1.set_title('Neuron Ablation Status by Layer')
    ax1.legend()
    ...
    ax2.bar(layers, ablated_pct, label='Ablated %', alpha=0.7)
    ax2.bar(layers, cannot_ablate_pct, bottom=ablated_pct, label='Cannot Ablate %', alpha=0.7)
    ax2.set_xlabel('Layer')
    ax2.set_ylabel('Percentage')
    ax2.set_title('Neuron Ablation Status by Layer (%)')
    ax2.legend()
...
def create_best_experiment_layer_distribution(experiments, save_path="best_experiment_layer_distribution.png", show_plot=True):
    """
    ...
    ax1.set_xlabel('Layer')
    ax1.set_ylabel('Neuron Count')
    ax1.set_title(f'Neuron Ablation Status by Layer\n(Best Experiment - Threshold: {threshold_value})')
    ax1.legend()
    ...
    ax2.set_xlabel('Layer')
    ax2.set_ylabel('Percentage')
    ax2.set_title(f'Neuron Ablation Status by Layer (%)\n(Best Experiment - Threshold: {threshold_value})')
    ax2.legend()
...
def create_evaluation_hits_at_1_plot(experiments, save_path="evaluation_hits_at_1.png", show_plot=True):
    ...
    for experiment_name, data in eval_experiments.items():
        ...
        experiment_names.append(experiment_name)
        v_eval_values.append(data.get('v_eval', None))
        t_eval_values.append(data.get('t_eval', None))
    ...
    if has_threshold_data:
        ...
        plt.xticks(x, [f'{val:.3f}' if isinstance(val, (int, float)) else str(val) for val in x_values], rotation=45, ha='right')
    else:
        # Use topic names for x-axis labels, but clean them up for display
        display_names = [name.split('_', 1)[1] if '_' in name else name for name in x_values]
...
def create_topic_statistics_plot(save_path="topic_statistics.png", show_plot=True):
    ...
    topic_data = [
        {'topic_name': '0_song_album_singles_chart', 'queries': 89, 'documents': 1000, 'confidence': 0.998, 'topic_id': 0},
        {'topic_name': '1_film_films_disney_movie', 'queries': 47, 'documents': 501, 'confidence': 0.978, 'topic_id': 1},
        {'topic_name': '14_bank_financial_business_banking', 'queries': 13, 'documents': 78, 'confidence': 0.894, 'topic_id': 14},
        {'topic_name': '2_season_series_tv_2017', 'queries': 17, 'documents': 205, 'confidence': 0.871, 'topic_id': 2},
        {'topic_name': '24_game_games_nintendo_xbox', 'queries': 14, 'documents': 59, 'confidence': 0.708, 'topic_id': 24},
        {'topic_name': '12_hai_dil_singh_ki', 'queries': 10, 'documents': 80, 'confidence': 0.698, 'topic_id': 12},
        {'topic_name': '9_india_indian_gandhi_party', 'queries': 12, 'documents': 89, 'confidence': 0.684, 'topic_id': 9},
        {'topic_name': '10_nerve_muscle_anterior_thyroid', 'queries': 12, 'documents': 85, 'confidence': 0.604, 'topic_id': 10},
        {'topic_name': '3_actor_he_episode_film', 'queries': 21, 'documents': 165, 'confidence': 0.587, 'topic_id': 3},
        {'topic_name': '21_olympic_olympics_games_skating', 'queries': 13, 'documents': 63, 'confidence': 0.553, 'topic_id': 21}
    ]
...
def main():
    ...
    parser.add_argument('--visualizations', 
                       type=str, 
                       nargs='+',
                       choices=['all', 'heatmap', 'layer_dist', 'similarity', 'ratio_dist', 'best_experiment', 'threshold_vs_cannot_ablate', 'topic_vs_cannot_ablate', 'topic_stats', 'evaluation_hits_at_1'],
                       default=['all'],
    ...
    if 'all' in args.visualizations:
        visualizations_to_run = ['heatmap', 'layer_dist', 'similarity', 'ratio_dist', 'best_experiment', 'threshold_vs_cannot_ablate', 'topic_vs_cannot_ablate', 'topic_stats', 'evaluation_hits_at_1']
    else:
    ...
    if 'heatmap' in visualizations_to_run:
        create_cannot_ablate_heatmap(experiments, str(visualizations_dir / "cannot_ablate_heatmap.png"), show_plot=not args.no_show)
    
    if 'layer_dist' in visualizations_to_run:
        create_layer_ablation_distribution(experiments, str(visualizations_dir / "layer_ablation_distribution.png"), show_plot=not args.no_show)
    
    if 'similarity' in visualizations_to_run:
        create_experiment_similarity_matrix(experiments, str(visualizations_dir / "experiment_similarity.png"), show_plot=not args.no_show)
    
    if 'ratio_dist' in visualizations_to_run:
        create_ablation_ratio_distribution(experiments, str(visualizations_dir / "ablation_ratio_distribution.png"), show_plot=not args.no_show)
    
    if 'best_experiment' in visualizations_to_run:
        create_best_experiment_layer_distribution(experiments, str(visualizations_dir / "best_experiment_layer_distribution.png"), show_plot=not args.no_show)
    
    if 'threshold_vs_cannot_ablate' in visualizations_to_run:
        create_threshold_vs_cannot_ablate_plot(experiments, str(visualizations_dir / "threshold_vs_cannot_ablate.png"), show_plot=not args.no_show)

    if 'topic_vs_cannot_ablate' in visualizations_to_run:
        create_topic_vs_cannot_ablate_plot(experiments, str(visualizations_dir / "topic_vs_cannot_ablate.png"), show_plot=not args.no_show)
    ...
    if 'evaluation_hits_at_1' in visualizations_to_run:
        create_evaluation_hits_at_1_plot(experiments, str(visualizations_dir / "evaluation_hits_at_1.png"), show_plot=not args.no_show)
